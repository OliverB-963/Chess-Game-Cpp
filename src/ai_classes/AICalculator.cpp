...
AIMove AICalculator::pickMove(Board boardCopy) {
    if (depth_ == 1) throw std::runtime_error("depth 1 not worth it");

    std::vector<AIMove> allMoves = generateMoves(boardCopy);

    boardCopy.swapPlayerTurn(true);

    for (AIMove &move : allMoves)
        explore(boardCopy, move, 2, boardCopy.playerTurn == PieceColour::White);

    return AIMove::maxChoice(allMoves, colour_);
}

void AICalculator::explore(Board board, AIMove &moveToExplore, int depth, bool maximising) { // NOLINT
    board.makeMove(board.getPiece(moveToExplore.piecePosition_), moveToExplore.moveToMake_);

    std::vector<AIMove> allMoves = generateMoves(board);

    if (depth == depth_) {
        moveToExplore.evaluation_ = boardEvaluationCalc(board); // Base Case
    }
    else { // Recursive Case

        board.swapPlayerTurn(true);

        for (AIMove &move : allMoves) {
            explore(board, move, depth + 1, !maximising);

        }

        moveToExplore.evaluation_ = AIMove::maxEvaluation(allMoves, board.playerTurn);
    }
}
